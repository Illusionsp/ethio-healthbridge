...
class TextQuery(BaseModel):
    text: str
    mode: Optional[str] = "adult"  # "adult" | "child"

response_cache = TTLCache(maxsize=100, ttl=3600)

@app.post("/api/voice-chat")
async def voice_chat(audio: UploadFile = File(...), mode: str = Form("adult")):
    ...
    try:
        ...
        if check_red_flags(transcribed_text):
            ...
            cache_key = f"{mode}:{enhanced_query}"
            if cache_key in response_cache:
                rag_response = response_cache[cache_key]
            else:
                rag_response = query_medical_guidelines(enhanced_query, mode=mode)
                response_cache[cache_key] = rag_response
...
async def text_chat(query: TextQuery):
    try:
        if check_red_flags(query.text):
            ...
            enhanced_query = f"{query.text} ({', '.join(extracted_symptoms)})" if extracted_symptoms else query.text

            cache_key = f"{query.mode}:{enhanced_query}"
            if cache_key in response_cache:
                rag_response = response_cache[cache_key]
            else:
                rag_response = query_medical_guidelines(enhanced_query, mode=query.mode)
                response_cache[cache_key] = rag_response
...
async def nearby_facilities(
    ...
    """Find nearest health facilities using OpenStreetMap (no API key needed)."""
    facilities = await find_nearby_facilities(lat, lon, radius)
